...
GENERATE_FROM_FLOW_PROMPT = """# כתיבת דרשה מלאה לפי מהלך מוגדר
...
## מהלך הדרשה
המשתמש בנה את המהלך הבא שלב-אחר-שלב. **עקוב אחריו בדיוק.**

{flow_sections}

## מפרשים משויכים לשלבים
{mefarshim_section}

## הנחיות
כתוב דרשה מלאה בעברית שעוקבת **בדיוק** אחרי המהלך שלמעלה.

לכל שלב במהלך:
- כתוב את התוכן כפי שמתואר ב-description
- השתמש במפרשים שמשויכים לשלב (אם יש)
- השתמש במעבר (transitionTo) כדי לעבור בצורה חלקה לשלב הבא
- שמור על הקצב — שלב עם 1 דקה צריך להיות קצר, שלב עם 3 דקות צריך להיות מפותח

הדרשה צריכה:
- לזרום כמו הרצאה/דרשה שנאמרת בפני קהל
- להשתמש בשפה חיה ומעניינת, לא אקדמית
- לנחות על הפאנצ'ליין בסוף
- אורך: 500-800 מילים

כתוב את הדרשה עצמה בלבד, בלי JSON, בלי כותרות טכניות.
"""
